  ...
  methods: {
    initSockit() {
      ...
      this.socket.addEventListener('message', (event = {}) => {
        ...
        const order = res[0] || {};
        console.log('WebSocket收到消息：', order);
        if( that.onRealtimeCallback) {
            ...
            that.bars =[{
                ...
                low: Number(order[3]),
                volume: Number(order[5]),
                time: Number(order[0] || new Date().getTime()),
            }]
            that.changePair()
        }
    ...
    getChartData(limit = 300) {
      let url = '/api/v5/market/candles?instId=BTC-USDT&bar=1m&limit=' + limit + '&after=' + new Date().getTime() + '&t=' + new Date().getTime();
